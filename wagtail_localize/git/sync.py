...
def _push(repo, logger):
    ...
    for translation in (
        ...
        resource = Resource.get_for_object(translation.object)

        # TODO: Only commit most up to date source
        source_po = generate_source_pofile(translation.source)
        source_po_filename = str(po_filename_for_object(resource))
        update_po(source_po_filename, source_po)

        locale_po = generate_locale_pofile(translation)
        update_po(
